...
#
#
#     if request.method == 'POST' and current_user == Blog.author :
#         Update = request.form.get('title')
#         Update = request.form.get('content')
#         db.session.commit()
#
...
def register():
    if request.method == 'POST':
        ...
        User.query.filter_by(password_hash=password_hash)
        if len(password) < 8:
            flash('PASSWORD SHOULD NOT BE LESS THAN 8', category='error')
            return redirect(url_for('register'))
